...
try {
    ...
    await Promise.all(resetBosses.map(boss =>
      axios.put(`http://localhost:3000/bosses/${boss.id}`, boss)
    ));
...
<template>
    <a  v-on:click.prevent="resetBeaten(boss)" class="inline-block bg-black text-white rounded-lg px-4 py-2 hover:bg-gray-700 button">
            Reset
        </a>
